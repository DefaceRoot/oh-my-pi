...
**Apply:** Get clear direction from the user during setup. Then iterate autonomously on implementation.

## 3. Metrics Must Be Mechanical

If you cannot verify with a command, you cannot iterate autonomously.
...
- **Human review** — user inspects the agent's decision sequence
...
**Apply:** At setup, explicitly state constraints. If the agent hits a wall it cannot solve (missing permissions, external dependency, needs human judgment), say so clearly instead of guessing.
...
This is not "removing humans." It is reassigning human effort from execution to direction. Humans become MORE valuable by focusing on irreducibly creative/strategic work.
