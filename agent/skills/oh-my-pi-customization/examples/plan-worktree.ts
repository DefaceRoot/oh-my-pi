/**
 * plan-worktree extension — reference implementation
 *
 * Demonstrates:
 * - Intercepting /plan via before_agent_start
 ...
 *
 * See ~/.omp/agent/extensions/plan-worktree/index.ts for the live version.
 */
...
export default function planWorktree(pi: ExtensionAPI) {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		if (!promptText.trimStart().toLowerCase().startsWith("/plan")) return;
		...
		try {
			...
			return {
				message: {
					customType: "plan-worktree/ready",
