...
function getUsageTokens(usage: unknown): number {
	...
	// Use uncached tokens for progress and UI counts.
	// Cached tokens (cacheRead/cacheWrite) can be huge and are not direct prompt/completion spend.
	const input = firstNumberField(record, ["input", "input_tokens", "inputTokens"]);
	const output = firstNumberField(record, ["output", "output_tokens", "outputTokens"]);
	if (input !== undefined || output !== undefined) {
		return (input ?? 0) + (output ?? 0);
	}

	// Fallback for providers that only emit a total count.
			...
			try {
				while (!submitResultCalled && retryCount < MAX_SUBMIT_RESULT_RETRIES && !abortSignal.aborted) {
					...
					const reminder = [
						...
						"- On success, call submit_result with: { result: { data: { ... } } }",
						"- On failure, call submit_result with: { result: { error: \"reason\" } }",
						"- result MUST contain exactly one of data or error (never both)",
						"- data MUST be a JSON object for successful completion (never a number or string)",
						"- Do NOT use placeholder numeric values (for example: data: 68)",
						"- If output schema fields are required, include them in result.data before submitting",
						"</submit-result-contract>",
					].join("\n");
