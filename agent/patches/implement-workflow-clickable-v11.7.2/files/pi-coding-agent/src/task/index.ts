...
import type { AgentTool, AgentToolResult, AgentToolUpdateCallback, ThinkingLevel } from "@oh-my-pi/pi-agent-core";
...
async function readModelRoleThinkingLevels(): Promise<Record<string, ThinkingLevel>> {
	try {
		const content = await fs.readFile(getModelRoleThinkingPath(), "utf8");
		...
		for (const [role, value] of Object.entries(parsed)) {
			if (typeof value === "string" && (VALID_THINKING_LEVELS as readonly string[]).includes(value)) {
				levels[role] = value as ThinkingLevel;
			}
		}
		return levels;
	} catch {
...
export class TaskTool implements AgentTool<TaskSchema, TaskToolDetails, Theme> {
	...
	public async execute(
		...
		const { agents, projectAgentsDir } = await discoverAgents(liveCwd);
		...
		const subagentRole = resolveSubagentRole(effectiveAgent.name);
		const effectiveAgentModel = isDefaultModelAlias(effectiveAgent.model) ? undefined : effectiveAgent.model;
		const resolveSubagentRuntimeOverrides = async (): Promise<{
			...
			const agentModelOverride = normalizeModelOverride(effectiveAgentModel);
			const activeSessionModelOverride = normalizeModelOverride(
				this.session.getActiveModelString?.() ?? this.session.getModelString?.(),
			);
			const modelOverride = roleModelOverride ?? agentModelOverride ?? activeSessionModelOverride;

			const roleThinkingLevels = await readModelRoleThinkingLevels();
			const roleThinkingLevel = roleModelLookupOrder
				.map(role => roleThinkingLevels[role])
				.find((level): level is ThinkingLevel => level !== undefined);
			const thinkingLevelOverride = roleThinkingLevel
				?? (roleModelOverride ? undefined : (effectiveAgent.thinkingLevel as ThinkingLevel | undefined));
			return { modelOverride, thinkingLevelOverride };
		};
		const initialRuntimeOverrides = await resolveSubagentRuntimeOverrides();
		const initialModelOverride = initialRuntimeOverrides.modelOverride;
		...
		if (agentName === "explore" && tasks.length > 5) {
			return {
				content: [
					{
						type: "text",
						text: "Explore fan-out limit exceeded: submit up to 5 explore tasks per call.",
					},
				],
				details: {
					projectAgentsDir,
		...
		if (isIsolated) {
			try {
				repoRoot = await getRepoRoot(liveCwd);
		...
		try {
			...
			const runTask = async (task: (typeof tasksWithSkills)[number], index: number) => {
				const { modelOverride, thinkingLevelOverride } = await resolveSubagentRuntimeOverrides();
				if (!isIsolated) {
