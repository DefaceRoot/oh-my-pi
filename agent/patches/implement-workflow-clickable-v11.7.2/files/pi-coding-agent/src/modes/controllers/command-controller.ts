...
export class CommandController {
	...
	handleHotkeysCommand(): void {
		...
		const subagentLeaderKey = this.ctx.keybindings.getDisplayString("cycleSubagentForward") || "Ctrl+X";
		const cycleAgentKey = this.ctx.keybindings.getDisplayString("cycleAgentMode");
		const cycleAgentHint = cycleAgentKey ? `${cycleAgentKey} (or ${subagentLeaderKey}+A)` : `${subagentLeaderKey}+A`;
		const hotkeys = `
**Navigation**
| Key | Action |
|-----|--------|
...
| \`${subagentLeaderKey}\` | Cycle subagent transcripts (open/close navigator modal) |
| \`Up/Down\` (or \`Left/Right\`) | Move between task subagent groups while navigator is open (most recent first) |
| \`Tab\` / \`Shift+Tab\` | Cycle nested subagents within the selected task group |
| \`Enter\` | Open selected subagent transcript from navigator |
