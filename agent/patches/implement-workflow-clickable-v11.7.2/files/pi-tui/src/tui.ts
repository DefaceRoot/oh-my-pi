...
export class TUI extends Container {
	...
	public onMouse?: (event: TerminalMouseEvent) => boolean | void;
	...
	private clearOnShrink = process.env.PI_CLEAR_ON_SHRINK !== "0"; // Clear empty rows when content shrinks (default: on, opt-out with PI_CLEAR_ON_SHRINK=0)
	...
	private handleInput(data: string): void {
		...
		if (mouseEvent) {
			const lineIndex = this.previousViewportTop + Math.max(0, mouseEvent.y - 1);
			const lineText =
			...
			// Always consume mouse events to avoid injecting raw escape sequences into editors.
			return;
		}
	 ...
	 * Searches for CURSOR_MARKER, calculates its position, and strips marker instances from all lines.
	 * Cursor position is only captured from markers in the bottom terminal-height viewport.
	 * @param lines - Rendered lines to search and sanitize
	 * @param height - Terminal height (visible viewport size)
	 * @returns Cursor position { row, col } or null if no marker found in viewport
	 */
	private extractCursorPosition(lines: string[], height: number): { row: number; col: number } | null {
		...
		for (let row = lines.length - 1; row >= 0; row--) {
			...
			if (!cursorPos && row >= viewportTop) {
				...
				cursorPos = { row, col: visibleWidth(beforeMarker) };
			}
			while (markerIndex !== -1) {
