...
import {
	...
	collectTaskUnitsFromTaskResultDetails,
	computeFilesDelta,
...
/**
 * implementation-engine extension
 *
...
export default function implementationEngine(pi: ExtensionAPI) {
	...
	let activeImplementationWorkerGate = false;
	let implementationWorkerGateState = createImplementationWorkerGateState();
	let implementationUnitScopeById = new Map<string, Set<string>>();
	...
	const captureGitStatusSnapshot = async (
		...
		if (status.code !== 0) {
			...
			pi.logger.warn("implementation-engine: failed to capture git status snapshot", {
				cwd,
				detail,
			});
	...
	const refreshImplementationWorkerBaseline = async () => {
		if (activeImplementationWorkerGate) {
			if (implementationWorkerBaselineCaptured) return;
			const snapshotCwd = normalizePath(
				last.worktreePath ?? last.repoRoot ?? process.cwd(),
			);
			implementationWorkerBaselineSnapshot = await captureGitStatusSnapshot(snapshotCwd);
			implementationWorkerBaselineCaptured = true;
			return;
		}
	...
	const mergeTaskUnitsForScope = (
		inputUnits: ReturnType<typeof collectTaskUnitsFromTaskInput>,
		resultUnits: ReturnType<typeof collectTaskUnitsFromTaskResultDetails>,
	) => {
		const byId = new Map<string, (typeof inputUnits)[number]>();
	...
	const captureImplementationWorkerOwnedFiles = async (): Promise<Set<string>> => {
		if (!activeImplementationWorkerGate) return new Set<string>();
		const snapshotCwd = normalizePath(
			last.worktreePath ?? last.repoRoot ?? process.cwd(),
		);
	...
	const tryRestoreWorktreeState = async (
		...
		for (let i = entries.length - 1; i >= 0; i--) {
			...
			pi.logger.debug("implementation-engine: restored worktree state", {
				worktreePath,
...
const notifyWorktreeProgress = (
			...
			if (manualMode) {
				...
				const manualKickoffTemplate = isFreeform
					? [
							...
							"- Each phase is delegated via Task subagents; run multiple implementation subagents in parallel only when scopes are proven independent. You never implement directly.",
							"- Phases run in strict order. Phase N+1 waits for all Phase N implementation Task subagents to complete.",
							...
							"- Verifier Workflow (MANDATORY): at each phase end, spawn parallel verifiers (one per completed implementation task in that phase).",
							"- Verifier checks MUST cover: lint passed on modified files, tests exist and pass, and success criteria are met.",
							"- Each verifier should write its verification report to: <session_workspace>/verification/<phase-or-task-name>.md",
					...
					: [
							...
							'- Verifier Workflow (MANDATORY): after each phase\'s implementation task batch completes, spawn parallel verifiers (one per completed implementation task). Each verifier assignment MUST check: (1) lint passed on modified files, (2) tests exist and pass, (3) success criteria met, and write a report to: <session_workspace>/verification/<phase-or-task-name>.md. The `qa-test-planner` skill is auto-injected.',
	...
	const launchUpdateVersion = async (
		...
		try {
			...
			const msg = err instanceof Error ? err.message : String(err);
			pendingUpdateVersionFinalize = false;
			pi.logger.error("implementation-engine: update-version-workflow failed", {
				error: msg,
			});
	...
	const finalizeUpdateVersionRun = async (ctx: ExtensionContext) => {
		...
		try {
			...
			setActionButton(ctx, "update-version");
			persistWorktreeState();
			pi.logger.error("implementation-engine: update-version finalize failed", {
				error: msg,
				branchName,
				worktreePath,
			});
	...
	const getOrchestratorPrompt = () => {
		...
		return [
			...
			'  1. After each phase\'s implementation task batch completes, spawn parallel verifiers (one per completed implementation task) and require each verifier assignment to check: (1) lint passed on modified files, (2) tests exist and pass, (3) success criteria met. Each verifier writes a report to: <session_workspace>/verification/<phase-or-task-name>.md. The `qa-test-planner` skill is auto-injected.',
			...
			"  1. At each phase end, after that phase's implementation task batch completes, spawn parallel verifiers (one per completed implementation task)",
	...
	const syncSessionRoleMarker = (
		...
		try {
			...
			pi.logger.debug("implementation-engine: synced session model role marker", {
				role,
				model: modelRef,
			...
			pi.logger.warn(
				"implementation-engine: failed to sync session model role marker",
				{
	...
	const applyThinkingLevelSafely = (
		ctx: ExtensionContext,
		thinkingLevel: OrchestratorThinkingLevel,
		...
		try {
			...
			if (/missing thinking metadata/i.test(message)) {
				const activeModel = ctx.model
					? `${ctx.model.provider}/${ctx.model.id}`
					: "(unknown)";
				pi.logger.warn(
					"implementation-engine: skipped thinking level sync because active model is missing thinking metadata",
					{
						...metadata,
						level: thinkingLevel,
						model: activeModel,
			...
			if (switched) {
				...
				if (roleMutator) {
					try {
						...
						pi.logger.warn(
							"implementation-engine: failed to preserve default model role after orchestrator switch",
							{
								error: err instanceof Error ? err.message : String(err),
							},
						);
					...
					pi.logger.warn(
						"implementation-engine: could not load settings mutator; orchestrator model switch may overwrite default model role",
					);
					missingModelRoleMutatorNotified = true;
				}
				pi.logger.debug(
					"implementation-engine: switched parent session to orchestrator model",
					{
			...
			const source = persistedLevels[roleName]
				...
				: embeddedLevel
					...
					: configuredDefaultThinkingLevel
						...
						: "env";
			const applied = applyThinkingLevelSafely(ctx, thinkingLevel, {
				stage: "ensure-orchestrator-runtime-defaults",
				role: roleName,
				source,
			});
			if (applied) {
				pi.logger.debug("implementation-engine: applied orchestrator thinking level", {
					role: roleName,
					level: thinkingLevel,
					source,
				});
	...
	const ensureDefaultRuntimeDefaults = async (
		...
		if (!alreadyActive) {
			...
			if (switched) {
				...
				pi.logger.debug(
					"implementation-engine: switched parent session to default model",
					{
		...
		const thinkingLevel =
			...
			configuredDefaultThinkingLevel;
		if (thinkingLevel) {
			const source = persistedLevels["default"]
				? "json-file"
				: embeddedLevel
					? "model-ref"
					: "config-defaultThinkingLevel";
			const applied = applyThinkingLevelSafely(ctx, thinkingLevel, {
				stage: "ensure-default-runtime-defaults",
				role: "default",
				source,
			});
			if (applied) {
				pi.logger.debug("implementation-engine: applied default role thinking level", {
					level: thinkingLevel,
					source,
				});
			}
	...
	> => {
		try {
			...
			pi.logger.debug("implementation-engine: repo root", { repoRoot });
			...
			if (last.repoRoot && last.worktreePath) {
				try {
					...
					pi.logger.warn(
						"implementation-engine: cleanup after setup failure encountered errors",
						{
							error:
			...
			ctx.ui.notify(`implementation-engine error: ${msg}`, "error");
			return;
		}
	};
	...
	const ensurePinnedToWorktree = async () => {
		...
		if (current !== expected) {
			...
			pi.logger.warn(
				"implementation-engine: cwd drift detected; re-pinned to worktree",
				{
					from: current,
					to: expected,
				},
	...
	const syncWorktreeBranchToRemote = async (
		...
		notifyWorktreeProgress(ctx, `remote-sync: ${reason}...`);
			...
			pi.logger.debug("implementation-engine: remote branch sync complete", {
				reason,
				branchName,
				repoRoot,
				worktreePath,
	...
	const deleteCurrentWorktree = async (
		...
		try {
			...
			const msg = err instanceof Error ? err.message : String(err);
			pi.logger.error("implementation-engine: delete-worktree failed", {
				error: msg,
				branchName,
				worktreePath,
			});
	...
	const continuePlannedWorktreeFromLinkedPlan = async (
		...
		try {
			...
			const msg = err instanceof Error ? err.message : String(err);
			pi.logger.error("implementation-engine: planned-worktree finalization failed", {
				error: msg,
			});
			...
			if (last.repoRoot && cleanupWorktreePath) {
				try {
					...
					pi.logger.warn(
						"implementation-engine: planned cleanup after failure encountered errors",
						{
							error:
								cleanupErr instanceof Error
									? cleanupErr.message
	...
	const suggestBranchNameFromPlan = async (
		...
		try {
			...
			if (!curatorSuggestedBranch) {
				pi.logger.warn(
					"implementation-engine: curator suggestion empty after sanitize; using fallback",
					{
			...
			if (!isStrictKebabCase(curatorSuggestedBranch)) {
				pi.logger.warn(
					"implementation-engine: curator suggestion invalid format after sanitize; using fallback",
					{
						curatorSuggestionText,
						fallback: fallbackSuggestion,
					},
			...
			if (curatorSuggestedBranch === "implement") {
				pi.logger.warn(
					"implementation-engine: curator suggestion too generic; using fallback",
					{
						curatorSuggestionText,
						fallback: fallbackSuggestion,
					},
			...
			const msg = err instanceof Error ? err.message : String(err);
			pi.logger.warn(
				"implementation-engine: curator timeout/failed/error; using fallback",
				{
					error: msg,
					fallback: fallbackSuggestion,
				},
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (ctx.hasUI) {
			...
			if (thinkingLevel) {
				...
				if (applied) {
					pi.logger.debug("implementation-engine: synced role thinking level on session start", {
						role: sessionRole ?? "default",
						level: thinkingLevel,
					});
				}
	...
	pi.on("session_switch", async (_event, ctx) => {
		...
		if (ctx.hasUI) {
			...
			if (thinkingLevel) {
				...
				if (applied) {
					pi.logger.debug("implementation-engine: synced role thinking level on session switch", {
						role: sessionRole ?? "default",
						level: thinkingLevel,
					});
				}
	...
	pi.registerCommand("freeform-worktree", {
		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				const msg = err instanceof Error ? err.message : String(err);
				pi.logger.error("implementation-engine: freeform-worktree failed", {
					error: msg,
				});
				const cleanupWorktreePath = setup?.worktreePath ?? last.worktreePath;
				if (last.repoRoot && cleanupWorktreePath) {
					try {
						...
						pi.logger.warn(
							"implementation-engine: freeform cleanup after failure encountered errors",
							{
								error:
									cleanupErr instanceof Error
										? cleanupErr.message
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (activeImplementationWorkerGate && event.toolName === "submit_result") {
			...
			if (!submitDecision.allowed) {
				...
				return {
					block: true,
					reason: submitDecision.reason ?? "Implementation workflow gate blocked submit_result.",
				};
			}
		}
		...
		if (event.toolName === "task") {
			...
			if (taskAgent === "verifier") {
				...
				if (missingSkills.length > 0) {
					...
					pi.logger.debug("implementation-engine: auto-injected verifier skills", {
				...
				if (contextLooksLikeFullPlan && rawCtx.length > 0) {
					...
					if (planRef) {
						...
						pi.logger.debug("implementation-engine: sanitized verifier context", {
		...
		if (!setupDone || !last.worktreePath) {
			...
			if (isDocsPlanMarkdownPath(resolved)) {
				...
				setActionButton(ctx, "implement");
				pi.logger.debug("implementation-engine: captured /plan-new metadata", {
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		if (hasSubmitResultDeadlock) {
			pi.logger.error(
				"implementation-engine: quality gate blocked by subagent submit_result deadlock",
			);
			if (ctx.hasUI) {
				ctx.ui.notify(
		...
		const spawnedAgent =
			typeof taskInput.agent === "string" ? taskInput.agent : undefined;
		const inputUnits = collectTaskUnitsFromTaskInput(taskInput);
		const resultUnits = collectTaskUnitsFromTaskResultDetails(event.details);
		const taskUnitsForScope = mergeTaskUnitsForScope(inputUnits, resultUnits);
		const implementationTaskScopeMetadata =
			spawnedAgent && taskUnitsForScope.length > 0
				? createImplementationTaskScopeMetadata({
					agent: spawnedAgent,
					units: taskUnitsForScope,
					changedFiles,
					baseBranch: last.baseBranch,
					worktreePath: last.worktreePath,
				})
				: undefined;
		let taskResultDetails: unknown = event.details;
		if (implementationTaskScopeMetadata) {
			const scopedUnitEntries = implementationTaskScopeMetadata.units
				.filter((unit) => unit.editedFiles.length > 0)
				.map((unit) => [unit.unitId, new Set(unit.editedFiles)] as const);
			if (scopedUnitEntries.length > 0) {
				implementationUnitScopeById = new Map(scopedUnitEntries);
			}
		...
		if (activeImplementationWorkerGate && isImplementationWorkerGateAgent(spawnedAgent)) {
			const gateOutcome = hasSubmitResultDeadlock
				? {
						success: false,
						reason: "subagent exited without calling submit_result",
					}
				: evaluateImplementationWorkerGateTaskResult(
						{
		...
		if (setupDone && last.worktreePath) {
			try {
				...
				pi.logger.error("implementation-engine: remote sync failed after task", {
					error: msg,
					branchName: last.branchName,
					worktreePath: last.worktreePath,
				});
				...
				pi.sendMessage({
					customType: "implementation-engine/remote-sync-failed",
					content: [
						"## Remote sync failed",
						"",
		...
		if (hasSubmitResultDeadlock) {
			const existingDetails =
				taskResultDetails &&
				typeof taskResultDetails === "object" &&
				!Array.isArray(taskResultDetails)
					? (taskResultDetails as Record<string, unknown>)
					: {};
			return {
				isError: true,
				details: {
		...
		if (taskResultDetails !== event.details) {
			return {
				isError: event.isError,
				details: taskResultDetails,
	...
	pi.on("agent_end", async (_event, ctx) => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : String(err);
			pi.logger.warn("implementation-engine: orchestrator auto-compact failed", {
				error: msg,
			});
			if (ctx.hasUI)
			...
			if (!hasTitle) {
				...
				if (userEntry) {
					...
					if (cleaned.length > 5) {
						...
						pi.logger.debug("implementation-engine: captured session title", {
							title: cleaned,
						});
					}
				}
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		if (isNativeHandoffPrompt) {
			activeParentRuntimeRole = "default";
			pi.logger.debug(
				"implementation-engine: bypassing orchestrator mode for native /handoff",
			);
		}
		...
		if (setupDone && last.worktreePath) {
			try {
				...
				if (activeAgentIsParentTurn) {
					...
					pi.logger.debug(
						"implementation-engine: skipping parent runtime default sync for subagent turn",
					);
				}
				setActionButton(ctx, actionButtonStage);
			} catch (err) {
				const msg = err instanceof Error ? err.message : String(err);
				pi.logger.error("implementation-engine: failed to prepare worktree session", {
					error: msg,
				});
				if (ctx.hasUI)
			...
			if (activeAgentIsParentTurn) {
				...
				if (thinkingLevel) {
					const applied = applyThinkingLevelSafely(ctx, thinkingLevel, {
						stage: "before-agent-start-non-worktree",
						role: lastModelRole ?? activeParentRuntimeRole,
					});
					if (applied) {
						pi.logger.debug("implementation-engine: applied non-worktree role thinking level", {
							role: lastModelRole ?? activeParentRuntimeRole,
							level: thinkingLevel,
						});
					}
...
type ActionButtonStage =
	...
	| "cleanup"
	| "none";
const PERSISTED_WORKTREE_STATE_TYPE = "implementation-engine/state";
...
const PERSISTED_PLAN_METADATA_TYPE = "implementation-engine/plan-new-metadata";
const PERSISTED_REVIEW_FINDINGS_TYPE = "implementation-engine/review-findings";
...
function buildImplementationKickoffPrompt(input: {
	...
	return [
		...
		"19. Require each phase Task subagent that modifies files to run a nested `lint` subagent scoped to its changed paths before reporting completion. Lint failures must be fixed and re-linted inside that phase task.",
		'20. Verifier Workflow (MANDATORY): after each phase\'s implementation task batch completes, spawn parallel verifiers (one per completed implementation task). Each verifier assignment MUST check: (1) lint passed on modified files, (2) tests exist and pass, (3) success criteria met. Each verifier writes its report to: <session_workspace>/verification/<phase-or-task-name>.md. The `qa-test-planner` skill is auto-injected.',
...
function truncateForPrompt(text: string, maxChars: number): string {
	...
	return `${text.slice(0, maxChars)}\n\n[...truncated to ${maxChars} chars by implementation-engine extension...]`;
}
