...
describe("verifier model role wiring", () => {
	...
	test("recognizes verifier as a first-class subagent role", async () => {
		...
		expect(content).toMatch(/const MODEL_ROLE_NAMES = new Set\(\[[\s\S]*"verifier"[\s\S]*\]\);/);
		expect(content).toMatch(/function resolveSubagentRole\(agentName: string\): string \{\s*return MODEL_ROLE_NAMES\.has\(agentName\) \? agentName : "subagent";\s*\}/s);
	});
});
