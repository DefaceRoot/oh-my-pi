...
describe("plan-worktree phase 9 footer hover/click glitch reproduction (RED)", () => {
	...
	test("startup patch health guard validates runtime footer mapping and no longer runs as a disabled stub", async () => {
		const source = await readExtensionSource();

		expect(source).not.toContain("workflow patch guard disabled locally");
		expect(source).toContain('"/freeform-worktree"');
		expect(source).toContain('"/planned-worktree"');
		expect(source).toContain('"/cleanup-worktrees"');
		expect(source).toMatch(/hasRequiredWorkflowFooterButtonMapping/);
		expect(source).toMatch(/const applyArgs = \["bash", WORKFLOW_PATCH_SCRIPT_PATH, "apply"\];/);
		expect(source).toMatch(/runAllowFail\(applyArgs, process\.cwd\(\), 180_000\)/);
	});
});
