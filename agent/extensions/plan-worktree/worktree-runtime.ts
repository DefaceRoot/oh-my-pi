...
export async function createWorktree(
	...
	try {
		if (await Bun.file(`${worktreePath}/.git`).exists()) {
			pi.logger.debug(
				`plan-worktree: worktree already exists at ${worktreePath}, skipping`,
			);
			return;
		}
	} catch {
	...
	if (branchCheck?.stdout.trim()) {
		pi.logger.debug(
			`plan-worktree: branch ${branchName} exists, adding worktree`,
		);
		await run(["git", "worktree", "add", worktreePath, branchName], repoRoot);
	} else {
		pi.logger.debug(
			`plan-worktree: creating branch ${branchName} from ${baseBranch}`,
		);
		await run(
...
export async function bestEffortLinkProjectAgentsSkills(
	...
	try {
		...
		pi.logger.debug(
			"plan-worktree: linked project .agents/skills into worktree",
			{
				sourceSkillsDir,
				targetSkillsDir,
			},
		...
		pi.logger.warn(
			"plan-worktree: symlink for project .agents/skills failed, attempting copy",
			{
				error: message,
				sourceSkillsDir,
				targetSkillsDir,
	...
	try {
		...
		pi.logger.warn(
			"plan-worktree: copied project .agents/skills into worktree after symlink failure",
			{
				sourceSkillsDir,
				targetSkillsDir,
			},
		...
		pi.logger.warn(
			"plan-worktree: failed to expose project .agents/skills in worktree",
			{
...
export async function bestEffortAuggieIndex(
	...
	if (enabledRaw && FALSE_ENV_VALUES.includes(enabledRaw)) {
		pi.logger.debug(
			"plan-worktree: skipping auggie indexing (disabled by env)",
		);
		return false;
	}
	...
	try {
		...
		if (version.code !== 0) {
			...
			pi.logger.warn(
				"plan-worktree: auggie not available for automatic indexing",
		...
		if (warmup.code !== 0) {
			...
			pi.logger.warn("plan-worktree: automatic auggie indexing failed", {
				detail,
				workspacePath,
			});
		...
		pi.logger.debug("plan-worktree: auggie index warmup completed", {
			workspacePath,
		});
