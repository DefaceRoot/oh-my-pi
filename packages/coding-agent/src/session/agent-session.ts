...
import {
	...
	type SessionMessageEntry as CompactionSessionMessageEntry,
...
export class AgentSession {
	...
	async #maintainAdvisorContext(incomingTokens: number): Promise<boolean> {
		...
		const pathEntries: CompactionSessionEntry[] = messages.map((message, i) => {
			const id = `msg-${i}`;
			const parentId = i > 0 ? `msg-${i - 1}` : null;
			const timestamp = String(message.timestamp || Date.now());

			if (message.role === "compactionSummary") {
				const storedFirstKeptEntryId = (message as CompactionSummaryMessage & { firstKeptEntryId?: unknown })
					.firstKeptEntryId;
				return {
					type: "compaction",
					id,
					...
					firstKeptEntryId: typeof storedFirstKeptEntryId === "string" ? storedFirstKeptEntryId : `msg-${i + 1}`,
			...
			return {
				...
				timestamp,
				message,
			} satisfies CompactionSessionMessageEntry;
		});
