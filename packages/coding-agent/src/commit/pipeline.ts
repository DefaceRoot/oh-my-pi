...
async function runLegacyCommitCommand(args: CommitCommandArgs): Promise<void> {
	...
	} = await resolveCommitRoleModel(settings, modelRegistry, primaryModel, primaryApiKey);
...
async function generateAnalysis(input: {
	...
	commitApiKey: string;
	commitThinkingLevel?: ThinkingLevel;
	commitSettings: {
	...
	if (
		...
		return runMapReduceAnalysis({
			...
			analysisThinkingLevel: input.commitThinkingLevel,
			diff: input.diff,
			stat: input.stat,
