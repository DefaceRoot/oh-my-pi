...
3. **Parallelize maximally.** Every set of edits with disjoint file scope MUST ship as one batch. This includes parallel `tdd-red` agents and parallel implementation `task` agents when they do not touch the same files or tightly coupled code paths. Serialize only when one subagent produces a contract (types, schema, shared module) the next consumes — and state the dependency when you do.
