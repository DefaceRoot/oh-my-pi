...
const interactiveModePath = path.join(import.meta.dir, "interactive-mode.ts");
const interactiveModeSource = fs.readFileSync(interactiveModePath, "utf8");
...
describe("interactive mode footer prompts", () => {
	test("includes required workflow action buttons", () => {
		const actionButtonsBlock = extractActionButtonsBlock(interactiveModeSource);
