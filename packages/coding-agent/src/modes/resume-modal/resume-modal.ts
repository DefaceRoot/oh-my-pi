...
export class ResumeModal extends Container {
  ...
  constructor(
    ...
    currentCwd: string,
    callbacks: ResumeModalCallbacks,
  ) {
    super();
  ...
  render(width: number): string[] {
    ...
    const mode = colSpec.showSidebar
      ? colSpec.showDetail
        ? "full"
        : "compact"
      : "narrow";
    ...
    const panelLines = composePanels(
      sidebarLines,
      listLines,
      detailLines,
      colSpec,
    ...
    if (this._state.focusPanel === "projects") {
      ...
      if (matchesKey(keyData, "up") || keyData === "k") {
        if (this._state.selectedSessionIndex > 0) {
          ...
          this._state.selectedProjectIndex -= 1;
          const prevProject =
            this._state.projects[this._state.selectedProjectIndex];
        ...
        if (this._state.selectedSessionIndex < sessionCount - 1) {
          ...
          !isNarrow &&
          this._state.selectedProjectIndex < this._state.projects.length - 1
        ) {
          this._state.selectedProjectIndex += 1;
          this._state.selectedSessionIndex = 0;
          this._state.scrollOffset = 0;
        ...
        (matchesKey(keyData, "right") || keyData === "l") &&
        this._state.layoutMode === "full"
      ) {
  ...
  private _syncScrollOffset(bodyHeight: number, sessionCount: number): void {
    ...
    if (
      this._state.selectedSessionIndex >=
      this._state.scrollOffset + bodyHeight
    ) {
      this._state.scrollOffset =
        this._state.selectedSessionIndex - bodyHeight + 1;
    }

    const maxOffset = Math.max(0, sessionCount - bodyHeight);
    this._state.scrollOffset = Math.max(
      0,
      Math.min(this._state.scrollOffset, maxOffset),
    );
  }
