...
import type { LayoutMode, ModalColumnSpec, ProjectGroup, ResumeSessionEntry, TimeColor } from "./resume-modal-types.js";
...
export function renderHeader(innerWidth: number, filterMode: boolean, filterText: string): string[] {
	...
	if (filterMode) {
		...
		const spaceCount = Math.max(0, innerWidth - 2 - visibleWidth(title) - visibleWidth(rightSide));
...
export function renderFooter(innerWidth: number, layoutMode: LayoutMode): string[] {
	let helpText = "";
	if (layoutMode === "full") {
		helpText = "↑↓ Navigate  Tab Focus  / Filter  Enter Resume  A Archived  Esc Close";
...
export function renderProjectSidebar(
	...
	for (let i = 0; i < visibleProjects.length; i++) {
		...
		const spaceCount = Math.max(0, width - visibleWidth(baseLine) - visibleWidth(count));
...
export function renderSessionList(
	...
	const visibleSessions = showArchived ? sessions : sessions.filter(session => !session.isArchived);
	const maxVisible = Math.max(0, height);
	const lines: string[] = [];
	const start = Math.max(0, Math.min(scrollOffset, Math.max(0, visibleSessions.length - maxVisible)));
	...
	for (let i = 0; i < maxVisible; i++) {
		...
		const worktreeBadge = session.worktreeGroup !== "main" ? `${accent("[wt]")} ` : "";
		...
		const roleRaw = role === "orchestrator" ? "orch" : role === "default" ? " def" : "   ?";

		const separator = dim(" · ");
		const rightSide = [
			colorizeTime(timeRaw, session.timeColor),
			dim(msgRaw),
			role === "orchestrator" ? warning(roleRaw) : role === "default" ? success(roleRaw) : dim(roleRaw),
		].join(separator);

		const leftSide = `${prefix}${worktreeBadge}${archivedBadge}${title}`;
		const spaceCount = Math.max(0, width - visibleWidth(leftSide) - visibleWidth(rightSide));
...
export function renderDetailPanel(session: ResumeSessionEntry | undefined, height: number, width: number): string[] {
	const lines: string[] = [];
	...
	const role = detectAgentMode(session.session);
	const roleText = role === "orchestrator" ? "Orchestrator" : role === "default" ? "Default" : "Unknown";
