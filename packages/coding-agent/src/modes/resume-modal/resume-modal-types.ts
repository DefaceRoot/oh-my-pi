...
const TEMP_SESSION_CWD_PATTERNS = ["/tmp/pi-auto-compaction-", "/tmp/pi-compaction-", "/tmp/pi-handoff-"];
...
export function enrichSessions(sessions: SessionInfo[]): ResumeSessionEntry[] {
	...
	return sessions.map(session => {
		const modifiedTime =
			session.modified instanceof Date ? session.modified.getTime() : new Date(session.modified).getTime();
...
export function buildProjectGroups(grouped: Map<string, SessionInfo[]>, currentCwd: string): ProjectGroup[] {
