...
import { AgentSession, resolveToolCallBatchCapForModel } from "./session/agent-session";
...
function cloneCappedAssistantContentBlock(
...
function cloneCappedAssistantMessage(message: AssistantMessage): AssistantMessage {
	return {
		...message,
		content: message.content.map(cloneCappedAssistantContentBlock),
		usage: {
			...message.usage,
...
function capToolCallBatchStream(
	stream: AssistantMessageEventStream,
	maxToolCalls: number,
	abort: () => void,
): AssistantMessageEventStream {
	const capped = new AssistantMessageEventStream();
	void (async () => {
		...
		try {
			for await (const event of stream) {
				...
				if (completedToolCalls < maxToolCalls) continue;
				abort();
				capped.push({
					type: "done",
					reason: "toolUse",
					message: cloneCappedAssistantMessage(event.partial),
				});
				break;
			}
			if (!capped.done) capped.end();
		} catch (error) {
			if (!capped.done) capped.fail(error);
		}
	})();
	return capped;
}
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		if (!model && !options.modelPattern) {
			...
			// the winning provider (for example anthropic's first catalog entry)
			// instead of the intended provider default. Mirrors findInitialModel's
			// precedence.
		...
		if (model) {
			...
			effectiveThinkingLevel = logger.time("resolveThinkingLevelForModel", () => {
				if (autoThinking) return resolveProvisionalAutoLevel(resolvedModel);
				const requestedThinkingLevel: ThinkingLevel | undefined =
					thinkingLevel === AUTO_THINKING ? undefined : thinkingLevel;
				return resolveThinkingLevelForModel(resolvedModel, requestedThinkingLevel);
			});
		...
		agent = new Agent({
			...
			streamFn: (streamModel, context, streamOptions) => {
				...
				const maxToolCalls = resolveToolCallBatchCapForModel(streamModel);
				...
				const upstreamSignal = streamOptions?.signal;
				return capToolCallBatchStream(
					streamSimple(streamModel, context, {
						...streamOptions,
						openrouterVariant: streamOptions?.openrouterVariant ?? openrouterVariant,
						signal: upstreamSignal
							? AbortSignal.any([upstreamSignal, capAbortController.signal])
							: capAbortController.signal,
					}),
					maxToolCalls,
